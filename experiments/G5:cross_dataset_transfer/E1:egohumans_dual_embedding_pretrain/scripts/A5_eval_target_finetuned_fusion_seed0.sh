#!/usr/bin/env bash
# Experiment Note: E1-A5-eval-target-finetuned-fusion-seed0
# Evaluate dual fusion with fine-tuned local + fine-tuned global branches.
set -euo pipefail
...
GLOBAL_EXP=$(cat "$ARTIFACTS_DIR/target_w24_seed${SEED}_global/stage2_exp_path.txt" 2>/dev/null || echo "")
OUT="$ARTIFACTS_DIR/target_w24_seed${SEED}_fusion/results_finetune_${SIM_NORM}.json"

if [[ -z "$LOCAL_EXP" || -z "$GLOBAL_EXP" ]]; then
    echo "[ERROR] Missing fine-tuned checkpoint for seed=$SEED"
    exit 1
fi

echo "=== E1 Fine-tuned fusion eval seed=$SEED (norm=$SIM_NORM) on $DEVICE ==="
echo "Local (fine-tuned): $LOCAL_EXP"
echo "Global (fine-tuned): $GLOBAL_EXP"
