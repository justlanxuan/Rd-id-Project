#!/usr/bin/env bash
# Experiment Note: E1-A3-train-source-global-one-seed
# Train Model-G (global) on EgoHumans for one seed.
set -euo pipefail
...
gpu_idx=${DEVICE##*:}
SEED_OUT="$ARTIFACTS_DIR/source_w24_seed${SEED}_global"
mkdir -p "$SEED_OUT"

echo "=== E1 Source Model-G seed=$SEED on GPU $gpu_idx ==="
...
conda run --no-capture-output -n mobind_repro python "$REPO/experiments/G3:custom_failure_diagnosis/E2:mobind_on_custom_same_split/scripts/generate_config.py" \
  --base "$CONFIG_DIR/stage1_global_base.yaml" \
  --seed "$SEED" \
...
conda run --no-capture-output -n mobind_repro python "$REPO/experiments/G3:custom_failure_diagnosis/E2:mobind_on_custom_same_split/scripts/generate_config.py" \
  --base "$CONFIG_DIR/stage2_global_base.yaml" \
  --seed "$SEED" \
...
echo "=== E1 Source Model-G seed=$SEED complete ==="
