# E1 Progress: EgoHumans Pre-Trained Dual-Embedding Transfer

## 当前状态

- [x] E1 formulation / plan 已创建。
- [x] A1: EgoHumans cache 已构建（`cache_action_1.2_0.5`，21,179 windows）。
- [x] A2/A3 source 训练脚本与 configs 已创建（已修复 stage1_exp_path.txt 缺失问题）。
- [x] A5 zero-shot 评估脚本已创建。
- [x] A6 target fine-tune 脚本已创建。
- [x] A3: Source Model-G seed0/1 完成。
- [🔄] A3: Source Model-G seeds 42/123 运行中（GPU 1/5）。
- [x] A2: Source Model-L seed0 完成。
- [🔄] A2: Source Model-L seed42 运行中（GPU 0）。
- [ ] A3: Source Model-G seeds 2/3 待启动。
- [ ] A2: Source Model-L seeds 123/1/2/3 待启动。
- [ ] A4: 源域融合评估待训练完成后执行。
- [x] A5: target zero-shot **seed0 已完成**：Mean FrameAcc = 0.2940（best α=1.0，pure local）。
- [ ] A5: target zero-shot 剩余 seeds 待 source 训练完成后执行。
- [🔄] A6: target fine-tune **Local seed0 已启动**（GPU 7）。
- [ ] A6: target fine-tune Global seed0 待 Local fine-tune 完成后执行。
- [ ] A8: 结果聚合待所有实验完成后执行。

## 训练状态

| Branch | Seed | GPU | Status | Best Val Acc |
|---|---|---|---|---|
| Global | 0 | — | 完成 | ~0.63 |
| Global | 1 | — | 完成 | — |
| Global | 42 | 1 | Stage1/2 运行中 | — |
| Global | 123 | 5 | Stage1/2 运行中 | — |
| Global | 2 | — | 待启动 | — |
| Global | 3 | — | 待启动 | — |
| Local | 0 | — | 完成 | ~0.64 |
| Local | 42 | 0 | 运行中 | — |
| Local | 123/1/2/3 | — | 待启动 | — |
| Local fine-tune | 0 | 7 | 运行中 | — |

## 下一步

1. 等待 Local fine-tune seed0 完成，评估其 FrameAcc。
2. 继续启动剩余 source seeds（利用释放的 GPU）。
3. 根据 fine-tune 结果决定是否调整策略。
