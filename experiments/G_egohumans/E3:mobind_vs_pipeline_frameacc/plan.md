...
- 两个指标概念相近但实现不同；本实验在相同的提取轨迹、相同的 GT-to-extract mapping、相同的时序框架下对齐它们。
...
### A2: Our Pipeline FrameAcc on the same subset
- 脚本：`scripts/A2_eval_ours_frameacc_subset.py`
- 输入：过滤后的 `windows_test.csv`（仅保留 MoBInd train-only 序列）
