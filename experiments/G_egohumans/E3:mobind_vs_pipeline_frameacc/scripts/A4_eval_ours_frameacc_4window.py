#!/usr/bin/env python3
"""
Experiment Note: A4-eval-ours-frameacc-4window
Run our own pipeline's synchronous eval with every 4 consecutive windows
aggregated into one matching decision. This makes the matching unit (~4.8s)
comparable to MoBInd's 5-second window.
"""
import argparse
import csv
import subprocess
...
def main():
    ...
    parser.add_argument("--output_json", default="/home/fzliang/Autism-project/experiments/G_egohumans/E3:mobind_vs_pipeline_frameacc/results/ours_frameacc_4window.json")
    ...
    print(f"Saved 4-window FrameAcc results to {args.output_json}")
