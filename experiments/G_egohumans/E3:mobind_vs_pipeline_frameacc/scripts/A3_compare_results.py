...
Load MoBInd and our-pipeline FrameAcc results (single-window and 4-window)
on the same 16 sequences, print side-by-side tables, and generate charts.
"""
import json
...
def main():
    ...
    if ours4_mean is not None:
        ...
        for entry, s in zip(summary["per_sequence"], seq_ids):
            entry["ours_4window"] = ours4_seq[s]

    with open(RESULTS_DIR / "frameacc_comparison.json", "w") as f:
        json.dump(summary, f, indent=2)

    # Bar chart: MoBInd vs Ours(1w) vs Ours(4w)
    x = np.arange(len(seq_ids))
    width = 0.25
    fig, ax = plt.subplots(figsize=(14, 5))
    ax.bar(x - width, mobind_vals, width, label="MoBInd")
    ax.bar(x, ours_vals, width, label="Ours (1 window)")
    if ours4_seq is not None:
        ours4_vals = [ours4_seq[s] for s in seq_ids]
        ax.bar(x + width, ours4_vals, width, label="Ours (4 windows)")
