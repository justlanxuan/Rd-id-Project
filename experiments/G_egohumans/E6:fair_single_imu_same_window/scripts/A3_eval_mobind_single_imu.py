#!/usr/bin/env python3
"""
Experiment Note: A3-eval-mobind-single-imu
Evaluate the retrained MoBInd Stage2 (single RightWrist IMU, 24-frame window)
on the 24 official MoBInd test sequences using the same synchronous-style
FrameAcc metric as our pipeline.
"""
import argparse
import json
import os
...
# Raw MoBInd extracted_data has 5 sensors in this order.
...
def main():
    ...
    parser.add_argument("--npz_root", default="/home/fzliang/Autism-project/data/interim/egohumans_mobind_aligned_single_imu_24/slice/sequences")
    parser.add_argument("--mobind_extracted_root", default="/data/lyxie/ReID/Data/egohumans/extracted_data")
    parser.add_argument("--mobind_exp_dir", required=True, help="Path to retrained MoBInd Stage2 experiment directory")
    ...
    # Do NOT override stage1_exp; the retrained Stage2 config already points to its Stage1 output.
    ...
    for seq_id, action, seq, npz_path in tqdm(selected, desc="MoBInd FrameAcc"):
        ...
        # Keep only the RightWrist sensor to match the single-IMU model.
        right_wrist_idx = MOBIND_SENSOR_ORDER.index("RightWrist")
        imu_raw = imu_raw[:, :, right_wrist_idx:right_wrist_idx + 1, :]
        pose2d = pose2d[:tlen]
        npz_trunc = {}
    ...
    summary = {
        "model": "MoBInd_stage2_single_imu_w24",
