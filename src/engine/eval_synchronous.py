...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--group_vote", action="store_true", help="If set with --group_windows > 1, use majority vote over per-window assignments instead of embedding averaging")
...
def evaluate_sequence(
    ...
    # ------------------------------------------------------------------
    # 3. Hungarian matching (per window)
    # ------------------------------------------------------------------
    per_window_centers: List[int] = []
    per_window_assignments: List[np.ndarray] = []
    for w_idx, (start, end, active_pred, imu_ids_for_win, skel_ids_for_win) in enumerate(window_meta):
        ...
        per_window_centers.append(center)
        per_window_assignments.append(assignment)

    # ------------------------------------------------------------------
    # 4. Aggregate into groups if requested
    # ------------------------------------------------------------------
    ...
    if group_windows == 1:
        centers = per_window_centers
        center_assignments = per_window_assignments
    else:
        num_w = len(per_window_centers)
        if group_vote:
            # Majority vote over per-window assignments
            for w in range(num_w - group_windows + 1):
                group_centers = per_window_centers[w:w + group_windows]
                group_assigns = per_window_assignments[w:w + group_windows]
                center = group_centers[group_windows // 2]
                assignment = np.full(N_imu, -1, dtype=np.int64)
                for i in range(N_imu):
                    votes = [a[i] for a in group_assigns if a[i] != -1]
                    if votes:
                        from collections import Counter
                        c = Counter(votes)
                        assignment[i] = c.most_common(1)[0][0]
                centers.append(center)
                center_assignments.append(assignment)
        else:
            # Average embeddings across the group
            for w in range(num_w - group_windows + 1):
    ...
    # ------------------------------------------------------------------
    # 5. Assign each frame to the nearest chunk center
    # ------------------------------------------------------------------
...
def main() -> None:
    ...
    for seq_id, npz_path in sequences:
        ...
        frame_assignments = evaluate_sequence(
            ...
            args.imu_lowpass_cutoff_hz, args.imu_lowpass_fs_hz, args.group_windows, args.group_vote
        )
