"""Preprocess for custom+ annotated dataset (batch_20260505_v02).
...
Outputs (compatible with existing custom pipeline):
  /data/fzliang/reid-project/custom_plus/preprocessed/
  - sequences/custom_plus_<segment_id>.npz
  - sequences/custom_plus_<segment_id>.json
  - video_manifest.csv
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--task", choices=["preprocess", "slice"], default="preprocess")
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/data/fzliang/reid-project/custom_plus/preprocessed/default",
    )
    parser.add_argument(
        "--min_persons",
        type=int,
...
def run_slice(args: argparse.Namespace) -> None:
    cfg = load_slice_cfg(getattr(args, "config", None))
...
def main() -> None:
    ...
    for segment_dir in sorted(raw_root.iterdir()):
        ...
        sequence_id = f"custom_plus_{segment_id}"
        ...
        for imu_path in imu_paths:
            try:
                ...
                if args.imu_lowpass_cutoff_hz is not None:
                    imu7 = lowpass_filter_fft(
                        imu7, args.imu_lowpass_cutoff_hz, args.imu_lowpass_fs_hz
                    )
        ...
        np.savez_compressed(
            ...
            dataset=np.array("custom_plus", dtype=object),
